...
Los links dentro del contenido de texto (posts, páginas estáticas) SHALL heredar el color del texto circundante y ser distinguibles únicamente por subrayado. El color `--color-accent` SHALL NOT aplicarse como `color` en elementos `<a>` dentro de `.page-body`.
...
El borde izquierdo de los blockquotes SHALL usar el color Blue de la paleta (`--wada-blue`, `#006eb8`), no el color de acento general. Este requisito aplica a los blockquotes dentro de `.page-body`.
...
Todo el espaciado vertical entre elementos de contenido editorial SHALL derivarse de un único token `--baseline`, calculado como el producto del tamaño de fuente base del breakpoint activo y el interlineado del cuerpo. Un espacio de un baseline equivale a una línea vacía visible entre bloques de texto. Los tokens de espacio genéricos (`--space-*`) SHALL NOT usarse para márgenes entre elementos de texto dentro de `.page-body`.
